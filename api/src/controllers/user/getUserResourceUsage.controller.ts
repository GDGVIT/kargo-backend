...
const getUserResourceUsage = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId =
      (req.user && ('id' in req.user ? (req.user as any).id : (req.user as any)._id)) ||
      req.params.id;
    if (!userId) {
      log({ type: 'error', message: 'User id required' });
      return res.status(400).json(formatNotification('User id required', 'error'));
    }
    ...
    if (user.plan && typeof user.plan === 'object' && 'resources' in user.plan) {
    ...
    const allowed = {
      requests: {
        ...
        memory: parse(planResources.requests?.memory) + parse(extra.requests?.memory),
        storage: parse(planResources.requests?.storage) + parse(extra.requests?.storage),
      },
      limits: {
        cpu: parse(planResources.limits?.cpu) + parse(extra.limits?.cpu),
        memory: parse(planResources.limits?.memory) + parse(extra.limits?.memory),
        storage: parse(planResources.limits?.storage) + parse(extra.limits?.storage),
      },
    };
