...
const setUsername = async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.isAuthenticated?.() || !req.user) {
      ...
      return res
        .status(401)
        .json(formatNotification('You must be logged in to set a username.', 'error'));
    }
    ...
    if (user.username) {
      ...
      return res
        .status(400)
        .json(formatNotification('Username is already set and cannot be changed.', 'warning'));
    }
    ...
    if (existing) {
      ...
      return res
        .status(400)
        .json(
          formatNotification('This username is already taken. Please choose another.', 'warning')
        );
    }
