...
const getApplicationMetrics = asyncHandler(async (req: Request, res: Response) => {
  ...
  for (const [key, query] of Object.entries(queries)) {
    const instantUrl = `${prometheusBaseUrl}/api/v1/query?query=${encodeURIComponent(query)}`;
    ...
    try {
      ...
      results[key] = {
        current: instantRes.data.data.result?.[0]?.value?.[1]
          ? Number(Number(instantRes.data.data.result[0].value[1]).toFixed(3)).toLocaleString(
              undefined,
              { maximumFractionDigits: 3 }
            )
          : null,
        history: rangeRes.data.data.result?.[0]?.values
          ? rangeRes.data.data.result[0].values.map(([ts, val]: [number, string]) => [
              ts,
