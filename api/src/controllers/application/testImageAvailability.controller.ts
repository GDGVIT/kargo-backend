...
const testImageAvailabilityController = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!imageUrl || typeof imageUrl !== "string") {
      ...
      return res
        .status(400)
        .json(
          formatNotification("Missing or invalid 'imageUrl' in body.", "error")
        );
    }
    ...
    try {
      ...
      log({
        type: "info",
        message: `Testing image availability: ${imageUrl}:${tag} (${
          credentials.length
        } credentials available${
          credentialIds ? `, filtered: ${credentialIds.join(", ")}` : ""
        })`,
      });
