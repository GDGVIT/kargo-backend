...
const runDockerHandler = async (req: Request, res: Response) => {
  ...
  if (!url || typeof url !== 'string') {
    ...
    return res.status(400).json(formatNotification("Missing or invalid 'url' in body.", 'error'));
  }
  try {
    ...
    res.status(200).json({
      ...formatNotification('Dockerfile and Compose generated successfully', 'success'),
