...
import { getNamespace, getResourceName } from '../../utils/k8s/helpers/k8sHelpers';
...
const updateApplication = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    if (resources) {
      ...
      if (quota.exceeded) {
        ...
        return res.status(400).json({
          ...formatNotification('Resource allocation exceeds your allowed quota.', 'warning'),
    ...
    res.status(500).json(formatNotification('Failed to update application', 'error'));
  }
});
