...
const streamApplicationLogs = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const appPods = pods.filter(pod => 
        pod.metadata?.labels?.app === app.name
      );
      
      const podNames = appPods.map(pod => pod.metadata?.name).filter(Boolean) as string[];
      ...
      const logStreams = podNames.map(async (podName) => {
        try {
          ...
          res.write(`data: ${timestamp} [${podName}] [Error getting logs: ${errorMsg}]\n\n`);
        }
      });
