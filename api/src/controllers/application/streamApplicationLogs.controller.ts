...
const streamApplicationLogs = asyncHandler(async (req: Request, res: Response) => {
  ...
  getPods.on('close', (_code: number) => {
    ...
    const logProcesses = podNames.map((podName) => {
      return {
        ...
        proc: spawn(
          'kubectl',
          ['logs', podName, '-n', namespace ?? 'default', `--tail=${LOG_TAIL_COUNT}`, '-f'],
          { stdio: ['ignore', 'pipe', 'pipe'] }
        ),
        buffer: '',
        sending: false,
    ...
    logProcesses.forEach((logObj) => {
      (logObj.proc.stdout as NodeJS.ReadableStream).on('data', (data: Buffer) => {
        logObj.buffer += data.toString();
        sendBufferedLogs(logObj);
      });
      (logObj.proc.stderr as NodeJS.ReadableStream).on('data', (data: Buffer) => {
        log({
          type: 'error',
