...
const applyApplication = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!manifestsDir) {
    ...
    return res.status(500).json(formatNotification('MANIFESTS_DIR not set in env', 'error'));
  }
  ...
  if (!app.namespace) {
    ...
    return res.status(500).json(formatNotification('Application namespace is undefined', 'error'));
  }
  ...
  if (pvcsYaml) {
    ...
    for (const pvcName of pvcNames) {
      try {
        ...
        log({
          type: 'warning',
          message: `PVC ${pvcName} could not be created: ${e}`,
        });
      }
    }
  }
