...
const githubRepos = async (req: Request, res: Response) => {
  try {
    ...
    if (req.query.installation_ids) {
      ...
      if (user && user.githubInstallationId && Array.isArray(user.githubInstallationId)) {
        const validIds = new Set(installationIds);
        const before = user.githubInstallationId.length;
        user.githubInstallationId = user.githubInstallationId.filter((id: string) =>
          validIds.has(id)
        );
      ...
      if (!user.githubInstallationId || user.githubInstallationId.length === 0) {
        log({ type: 'warning', message: 'GitHub not connected for user' });
        return res.status(400).json(formatNotification('GitHub not connected for user', 'warning'));
      }
    ...
    if (!Array.isArray(installationIds) || installationIds.length === 0) {
      ...
      return res
        .status(400)
        .json(formatNotification('Invalid or missing installation IDs', 'error'));
    }
    ...
    for (const installationId of installationIds) {
      try {
        ...
        if (error.response && [404, 401, 403].includes(error.response.status)) {
          ...
          installationIds = installationIds.filter((id: string) => id !== installationId);
    ...
    if (removedInstallationIds.length > 0) {
      log({
        type: 'warning',
        message: 'Some GitHub installations were invalid and have been removed.',
      });
