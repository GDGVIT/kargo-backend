...
const githubSaveInstallationID = async (req: Request, res: Response) => {
  ...
  if (!user) {
    ...
    return res.status(401).json(formatNotification('Not authenticated', 'error'));
  }
  if (!installation_id) {
    log({ type: 'error', message: 'Missing installation ID' });
    return res.status(400).json(formatNotification('Missing installation ID', 'error'));
  }
  ...
  try {
    ...
    res.status(200).json(formatNotification('Installation ID saved.', 'success'));
  } catch (err) {
    log({ type: 'error', message: 'Error saving installation ID', meta: err });
    res.status(500).json(formatNotification('Failed to save installation ID.', 'error'));
  }
};
