...
export default function generateK8sManifests(app: IApplication): Record<string, string> {
  ...
  if (app.resources?.requests?.storage && app.resources.requests.storage > 0) {
    // Convert bytes to GB
    storageGB = Math.ceil(app.resources.requests.storage / (1024 * 1024 * 1024));
  } else if (app.resources?.limits?.storage && app.resources.limits.storage > 0) {
    // Convert bytes to GB
  ...
  const pvManifests = volumes
    ...
    .filter((yaml) => yaml);
  const pvcManifests = volumes.map((v) => generatePVC(v, namespace)).filter((yaml) => yaml);
