...
function generateNodeSelectorBlock(sanitizedApp: any): string {
  if (!sanitizedApp.nodeSelector || Object.keys(sanitizedApp.nodeSelector).length === 0) {
    return '';
  }
...
function generatePortsBlock(ports: any[]): string {
  ...
  return (
    ...
    ports
      .map(
        (p: { name?: string; containerPort: number; protocol?: string }, idx: number) =>
          `            - name: port${idx}` +
...
function generateProbeBlock(type: 'readinessProbe' | 'livenessProbe', probe: any): string {
  if (!probe) return '';
