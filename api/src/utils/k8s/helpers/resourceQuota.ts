...
export async function checkResourceQuota({
  ...
  if (userModel) {
    ...
    if (userModel.plan && typeof userModel.plan === 'object' && 'resources' in userModel.plan) {
    ...
    const allowed = {
      requests: {
        cpu: (
          parseResource(planResources.requests?.cpu) + parseResource(extra.requests?.cpu)
        ).toString(),
        memory: (
          parseResource(planResources.requests?.memory) + parseResource(extra.requests?.memory)
        ).toString(),
      },
      limits: {
        cpu: (
          parseResource(planResources.limits?.cpu) + parseResource(extra.limits?.cpu)
        ).toString(),
        memory: (
          parseResource(planResources.limits?.memory) + parseResource(extra.limits?.memory)
        ).toString(),
      },
    };
